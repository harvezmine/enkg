#!/usr/bin/env bash
# deploy.sh — tarik kode terbaru situs Every Nation Kelapa Gading dari GitHub
# lalu deploy.
#
...
#   2. git merge --ff-only.
#   3. deploy/deploy.sh — npm ci, npm test, build, rakit rilis baru, tukar
#      symlink `current`, pm2 reload (worker diganti satu per satu, tanpa
#      downtime).
#   4. Cek kesehatan: situs menjawab 200 dan semua worker PM2 online.
#   5. Catat commit yang sukses ke $APP_DIR/.deployed-rev.
#
# Kalau test atau build gagal, symlink `current` belum ditukar, jadi rilis
...
$(git diff --name-only "$local_rev" "$remote_rev")"
  ...
  # Dicek setelah merge: commit yang baru di-pull bisa membawa versi barunya.
  [[ -x deploy/deploy.sh ]] || fail "deploy/deploy.sh tidak ada atau tidak executable"

  # --- test, build & rilis -------------------------------------------------
  log "Test, build & rilis $target_rev (npm ci, npm test, next build, tukar symlink, pm2 reload)..."
  ENKG_APP_DIR="$APP_DIR" deploy/deploy.sh 2>&1 | tee -a "$LOG_FILE" \
    || fail "test/build/rilis gagal — rilis lama tetap melayani. Commit $target_rev belum dicatat, jalankan ulang setelah diperbaiki."
